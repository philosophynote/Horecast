...
class HorseResults:
    ...
    def preprocessing(self):
        ...
        self.target_list = ['date', "course_len",
                            'rank', "odds", 'final_corner', "time_index"]
    ...
    def feature_processing(self, horse_id_list, date):
        ...
        self.process_dict["_3R_before"] = filtered_df.groupby(["horse_id"])[self.target_list[2:]].nth(2).add_suffix(
            "_3R_before")
        self.course_len_all = filtered_df.groupby(
            level=0)["course_len"].mean().rename("course_len_all")
    ...
    def merge(self, results, date):
        ...
        merged_df = df.merge(self.course_len_all,
                             left_on='horse_id', right_index=True, how='left')
        df_tmp_time_index = merged_df[[
            "time_index_1R_before", "time_index_2R_before", "time_index_3R_before"]]
        df_time_index = df_tmp_time_index.mean(axis='columns')
        merged_df["time_index_ave"] = df_time_index
        merged_df["time_rank_p"] = merged_df.groupby(
            level=0)["time_index_ave"].rank(ascending=False, method="max")
        merged_df.drop(["time_index_1R_before", "time_index_2R_before",
                       "time_index_3R_before", "time_index_ave"], inplace=True, axis=1)
        return merged_df
