...
def insert_predtable(pred_table):
    ...
    insert["id"] = insert['race_id'].str.cat(
        insert['horse_number'].astype(str).str.zfill(2))
...
def search_sql(race_date, race_park, race_number):
    ...
    race_df = race_df.query(
        'race_date == @race_date and race_park == @race_park and race_number == @race_number')
    ...
    df = race_df.merge(horse_df, left_on="race_id",
                       right_on="race_id", how="inner")
    df = df.merge(pred_table, left_on=["race_id", "horse_number"], right_on=[
                  "race_id", "horse_number"], how="inner")
    df_pre = df[["race_park", "race_name", "race_number", "race_turn", "course_len", "weather", "race_type", "race_condition",
                 "n_horses", "horse_number", "horse_name", "sex_age", "jockey_name", "jockey_weight", "pred", "center", "bet"]]
    df["horse_number"] = df["horse_number"].astype(str)
    df = df.merge(result, left_on=["race_id", "horse_number"], right_on=[
                  "race_id", "horse_number"], how="left")
    df_lat = df[["rank", "horse_number", "horse_name", "sex_age",
                 "jockey_name", "jockey_weight", "favorite", "odds"]]
    ...
    df_umaren = umaren.merge(pred_table, how="inner", left_on=[
                             "race_id"], right_on=["race_id"])
    ...
    umaren_df = pd.DataFrame(list(umaren_dict.items()),
                             columns=['race_id', 'umaren'])

    df_umatan = umatan.merge(pred_table, how="inner", left_on=[
                             "race_id"], right_on=["race_id"])
    ...
    umatan_df = pd.DataFrame(list(umatan_dict.items()),
                             columns=['race_id', 'umatan'])

    df_sanrenpuku = sanrenpuku.merge(pred_table, how="inner", left_on=[
                                     "race_id"], right_on=["race_id"])
    race_id_list = df_sanrenpuku["race_id"].unique()
    sanrenpuku_money = [bet_sanrenpuku(
        df_sanrenpuku, race_id) for race_id in race_id_list]
    sanrenpuku_dict = dict(zip(race_id_list, sanrenpuku_money))
    sanrenpuku_df = pd.DataFrame(list(sanrenpuku_dict.items()), columns=[
                                 'race_id', 'sanrenpuku'])

    df_sanrentan = sanrentan.merge(pred_table, how="inner", left_on=[
                                   "race_id"], right_on=["race_id"])
    race_id_list = df_sanrentan["race_id"].unique()
    sanrentan_money = [bet_sanrentan(df_sanrentan, race_id)
                       for race_id in race_id_list]
    sanrentan_dict = dict(zip(race_id_list, sanrentan_money))
    sanrentan_df = pd.DataFrame(list(sanrentan_dict.items()), columns=[
                                'race_id', 'sanrentan'])

    df_re = race_df.merge(umaren_df, how="left",
                          right_on="race_id", left_on="race_id")
    df_re = df_re.merge(sanrenpuku_df, how="left",
                        right_on="race_id", left_on="race_id")
    df_re = df_re.merge(umatan_df, how="left",
                        right_on="race_id", left_on="race_id")
    df_re = df_re.merge(sanrentan_df, how="left",
                        right_on="race_id", left_on="race_id")

    df_re = df_re[["race_number", "umaren",
...
def insert_race_card(df):
    race_df = df[["race_id", "race_park", "race_name", "race_number", "date",
                  "race_turn", "course_len", "weather", "race_type", "race_condition", "n_horses"]]
    ...
    horse_df.rename(columns={'枠': 'frame_number', '馬番': 'horse_number', '馬名': 'horse_name',
                    '性齢': "sex_age", '騎手': 'jockey_name', '斤量': 'jockey_weight'}, inplace=True)
    race_df.drop_duplicates(inplace=True)
    race_df = race_df.reset_index(drop=True)
    race_df.set_index("race_id", inplace=True)
    horse_df["id"] = horse_df['race_id'].str.cat(
        horse_df['horse_number'].astype(str).str.zfill(2))
...
def insert_result(race_id_list):
    ...
    for race_id in race_id_list:
        ...
        # Jupyterで停止ボタンを押した時の対処
            ...
            break
    data["id"] = data['race_id'].str.cat(
        data['horse_number'].astype(str).str.zfill(2))
...
def umaren(df):
    new_df = pd.DataFrame(
        columns=['win_1', 'win_2', 'win_3', 'win_4', 'return_1', 'return_2'])
    umaren = df[df[0] == '馬連'][[1, 2]]
    if umaren[2].str.contains('br').any():
        new_df[['win_1', 'win_2', 'win_3', 'win_4']] = umaren[1].str.split(expand=True)[
            [0, 1, 2, 3]]
        new_df[['return_1', 'return_2']] = umaren[2].str.split('br', expand=True)[
            [0, 1]]
        ...
        new_df.apply(lambda x: pd.to_numeric(
            x.str.replace(',', ''), errors='coerce'))
...
def umatan(df):
    ...
    if umatan[2].str.contains('br').any():
        new_df[['win_1', 'win_2', 'win_3', 'win_4']] = umatan[1].str.split(expand=True)[
            [0, 1, 2, 3]]
        new_df[['return_1', 'return_2']] = umatan[2].str.split('br', expand=True)[
            [0, 1]]
...
def sanrenpuku(df):
    ...
    df = df.rename(columns={'win_0': 'win_1',
                   'win_1': 'win_2', 'win_2': 'win_3'})
...
def sanrentan(df):
    new_df = pd.DataFrame(columns=[
                          'win_1', 'win_2', 'win_3', 'win_4', 'win_5', 'win_6', 'return_1', 'return_2'])
    rentan = df[df[0] == '3連単'][[1, 2]]
    if rentan[2].str.contains('br').any():
        new_df[['win_1', 'win_2', 'win_3', 'win_4', 'win_5', 'win_6']
               ] = rentan[1].str.split(expand=True)[[0, 1, 2, 3, 4, 5]]
        new_df[['return_1', 'return_2']] = rentan[2].str.split('br', expand=True)[
            [0, 1]]
        ...
        new_df = new_df.rename(
            columns={'win_0': 'win_1', 'win_1': 'win_2', 'win_2': 'win_3'})
        new_df.apply(lambda x: pd.to_numeric(
            x.str.replace(',', ''), errors='coerce'))
        new_df.dropna(subset=["win_1"], inplace=True)
        new_df.index.name = "race_id"
